...
public class CommonCard3 extends CommonCard{
    ...
    /**
     * Method that checks if there are four groups each containing at least4 tiles of the same type
    ...
    public boolean checkObjective(TileType[][] library) {
        ...
        for(int column = 0; column < WIDTH ; column++){
            for(int row = 0; row < HEIGHT ; row++){
                //For each tile (if it is still not used) we check the "horizontal" and "vertical" size
                ...
                //If the UpSize is exactly 4 AND BesidesSize exactly 1 (Vertical combination)
                // OR vice-versa (Horizontal combination) --> The group is ok!!
                if((sizeUp == 4 && sizeBeside == 1) || (sizeUp == 1 && sizeBeside == 4))
                    numGroups++;
